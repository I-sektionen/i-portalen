    ...
    if request.method == 'POST':
        ...
        if user is None:
            ...
            if user_account.exists() and user_account[0].last_login is None and user_account[0].is_active:
                ...
                messages.info(request, _("Du måste aktivera ditt konto innan du kan logga in första gången."))
                return redirect("password_reset", liu_id=username)
            else:
                # the authentication system was unable to verify the username and password
                messages.error(request, _("Fel Liu-id eller lösenord. Om felet kvarstår kontakta webgroup@isektionen.se"))
            return render(request, "user_managements/login.html")
    ...
    if request.method == 'POST' and 'member' in request.POST:
        ...
        if form.is_valid():
            ...
            messages.info(request,
                          _("Tack för att du vill vara medlem i sektionen."))
            if user.must_edit:
                form = ChangeUserInfoForm(instance=user)
                return render(request, "user_managements/force_user_form.html", {"form": form})
            auth_login(request, user)
        ...
        if form.is_valid():
            ...
            for liu_id in list_of_liu_id:
                ...
                try:
                    ...
                    messages.info(
                        request,
                        "".join([_("Det finns redan en användare med Liu-id:"),
                                 " {:}. ",
                                 _("Användaren har inte ändrats.")]).format(liu_id))
                    continue
                try:
            ...
            if errors:
                messages.info(request, _("De användare utan fel har skapats och kan nu återställa sitt lösenord."))
            else:
                messages.success(request, _("De nya användarna har skapats och kan nu återställa sitt lösenord"))
    else:
...
def update_user_from_kobra(request, liu_id):
    if not request.user.has_perm("user_managements.add_iuser"):
        messages.error(request, _("Du har inte rätt behörighet att updatera från kobra."))
    ...
    try:
        ...
        messages.error(request, _("Personen finns inte i systemet."))
    except LiuNotFoundError:
...
def update_all_users_from_kobra(request):
    ...
    for user in users:
        ...
        try:
            ...
            errors += (user.username + _(" finns inte i systemet.\n"))
        except LiuNotFoundError:
            errors += (user.username + _(" kan inte ansluta till kobra.\n"))
        except LiuGetterError:
            errors += (user.username + _(" fel i anslutingen till kobra.\n"))
    return render(request, "user_managements/kobra.html")
        ...
        if form.is_valid():
            ...
            for liu_id in list_of_liu_id:
                ...
                try:
                    ...
                    messages.error(request, liu_id + _(" finns inte i systemet.\n"))
                except LiuNotFoundError:
                    messages.error(request, liu_id + _(" kan inte ansluta till kobra.\n"))
                except LiuGetterError:
                    messages.error(request, liu_id + _(" fel i anslutingen till kobra.\n"))
    else:
        form = AddWhiteListForm()
    return render(request, "user_managements/add_whitelist.html", {'form': form})
...
def subscribe_to_ipikure(request):
    if not (request.user.address and request.user.zip_code and request.user.city):
        messages.error(request, _("Du måste ange din adress för att kunna prenumerera på ipikuré"))
    ...
    try:
        ...
        subscriber.save()
        messages.info(request, _("Du har nu uppdaterat din prenumeration av Ipikuré"))
    except IpikureSubscriber.DoesNotExist:
        IpikureSubscriber.objects.create(user=request.user)
        messages.info(request, _("Du prenumererar nu på Ipikuré"))
    return redirect(reverse("user_management:my page"))
